...
- **分配内存**：根据Java堆**是否规整**决定分配方式，分配方式包含两种：
  ...
  - TLAB：在线程初始化时默认分配空间，JVM在线程中分配内存时先在TLAB分配，不存在线程的竞争。当分配对象大雨TLAB剩余内存时，采用CAS+失败重试的方法创建；
...
- 对象头：
  - 运行时数据：如哈希码，GC分代年龄，锁状态标志位；
  - 类型指针：指向类元素据的指针，确定对象是哪个类的实例；
- 实例数据：真正存储对象有效信息的部分；
...
- 根据当前的执行任务是CPU密集型还是IO密集型：如果是CPU密集型应用，则线程池大小设置为CPU数量 + 1，如果是IO密集型应用，则线程池大小设置为2 * CPU数量 + 1，该设置偏理论方式；
